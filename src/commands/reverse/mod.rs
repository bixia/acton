...
fn validate_state_flow_tx_cell_consistency(
    ...
    validate_rand_seed_hex_consistency(
        &format!("{prefix} replay randSeedHex"),
...
fn validate_rand_seed_hex_consistency(
    label: &str,
    rand_seed_hex: &str,
    tx_hash: &str,
    gate_failures: &mut Vec<String>,
) {
    if !hex::decode(rand_seed_hex).is_ok_and(|bytes| bytes.len() == 32) {
        gate_failures.push(format!(
            "{label} for {tx_hash} is not a 32-byte random seed"
        ));
    }
}
...
fn validate_state_snapshot_cell_consistency(
    ...
    validate_shard_account_boc_decodable_consistency(
        &format!("{label} shardAccountBoc64"),
        &snapshot.shard_account_boc64,
        tx_hash,
        gate_failures,
    );
...
fn validate_shard_account_boc_decodable_consistency(
    label: &str,
    boc64: &str,
    tx_hash: &str,
    gate_failures: &mut Vec<String>,
    ...
    let Ok(cell) = Boc::decode_base64(boc64) else {
        gate_failures.push(format!(
            "{label} for {tx_hash} is not a decodable shard account"
        ));
        return;
    };
    if cell.parse::<ShardAccount>().is_err() {
        gate_failures.push(format!(
            "{label} for {tx_hash} is not a decodable shard account"
        ));
    }
...
fn validate_replay_observation_evidence_keys(
    ...
    validate_rejected_replay_observation_has_no_success_artifacts(
        observation,
        &observation_prefix,
        gate_failures,
    );
...
fn validate_rejected_replay_observation_has_no_success_artifacts(
    ...
    for (key, label) in [
        ...
        if observation.get(key).is_some_and(|value| !value.is_null()) {
            gate_failures.push(format!(
                "{observation_prefix} rejected observation must not include {label} evidence"
            ));
        }
    }
    for (key, label) in [("outbound", "outbound"), ("outActions", "outActions")] {
        if observation
            .get(key)
            .and_then(|value| value.as_array())
...
fn validate_replay_error_evidence_keys(
    ...
    {
        gate_failures.push(format!(
            "{observation_prefix} accepted observation must not include error evidence"
        ));
        return;
    }
    if observation
...
mod tests {
    ...
    #[test]
    fn artifact_manifest_validation_rejects_accepted_replay_error() {
        let temp_dir = tempfile::tempdir().expect("temp dir should be created");
        write_sample_validation_artifacts(temp_dir.path());
        let replay_path = temp_dir.path().join("target-a/replay.json");
        let mut replay: serde_json::Value = serde_json::from_str(
        ...
        .expect("replay artifact should parse");
        replay["replay"]["error"] = serde_json::json!({
            "message": "unexpected success error",
        ...
        assert!(
            validation.gate_failures.iter().any(|failure| {
                failure.contains(
                    "replay artifact target-a/replay.json replay accepted observation must not include error evidence",
                )
            }),
    ...
    #[test]
    fn artifact_manifest_validation_rejects_rejected_replay_success_artifacts() {
        let temp_dir = tempfile::tempdir().expect("temp dir should be created");
        write_sample_validation_artifacts(temp_dir.path());
        let replay_path = temp_dir.path().join("target-a/replay.json");
        let mut replay: serde_json::Value = serde_json::from_str(
        ...
        assert!(!validation.passed);
        for expected in [
            "replay artifact target-a/replay.json replay rejected observation must not include state evidence",
            "replay artifact target-a/replay.json replay rejected observation must not include compute evidence",
            "replay artifact target-a/replay.json replay rejected observation must not include money evidence",
        ] {
            assert!(
                validation
                    .gate_failures
                    .iter()
                    .any(|failure| failure.contains(expected)),
                "expected rejected replay success artifact failure {expected}, got {:?}",
                validation.gate_failures
            );
        }
    }
    ...
    #[test]
    fn state_flow_corpus_validation_rejects_malformed_transaction_shard_account_boc() {
        let temp_dir = tempfile::tempdir().expect("temp dir should be created");
        let corpus_path = temp_dir.path().join("corpus.json");
        let mut corpus: serde_json::Value =
            serde_json::from_str(&sample_replay_corpus_json()).expect("sample corpus parses");
        corpus["transactions"][0]["state"]["post"]["shardAccountBoc64"] =
        ...
        assert!(
            gate_failures.iter().any(|failure| {
                failure.contains("corpus artifact corpus.json post shardAccountBoc64")
                    && failure.contains("for tx-a is not a decodable shard account")
            }),
            "expected malformed corpus transaction shard account failure, got {:?}",
            gate_failures
        );
    }
    ...
    #[test]
    fn state_flow_corpus_validation_rejects_malformed_rand_seed() {
        let temp_dir = tempfile::tempdir().expect("temp dir should be created");
        let corpus_path = temp_dir.path().join("corpus.json");
        let mut corpus: serde_json::Value =
            serde_json::from_str(&sample_replay_corpus_json()).expect("sample corpus parses");
        corpus["transactions"][0]["replay"]["randSeedHex"] = serde_json::json!("ff");
        fs::write(&corpus_path, corpus.to_string()).expect("corpus artifact should be written");
        let artifact = super::SmokeArtifactManifestEntry::new(
            "corpus",
            "corpus.json",
        ...
        assert!(
            gate_failures.iter().any(|failure| {
                failure.contains("corpus artifact corpus.json replay randSeedHex")
                    && failure.contains("for tx-a is not a 32-byte random seed")
            }),
            "expected malformed corpus random seed failure, got {:?}",
            gate_failures
        );
    }
    ...
    fn artifact_manifest_validation_rejects_transaction_replay_context_mismatch_with_corpus() {
        ...
        assert!(
            validation.gate_failures.iter().any(|failure| {
                failure.contains("target-a: transaction replay rand seed ff for tx-a does not match corpus replay rand seed")
            }),
    ...
    #[test]
    fn state_flow_tx_validation_rejects_malformed_shard_account_boc() {
        let temp_dir = tempfile::tempdir().expect("temp dir should be created");
        let tx_path = temp_dir.path().join("transaction.json");
        let mut tx = sample_state_flow_json("tx-a");
        tx["state"]["post"]["shardAccountBoc64"] = serde_json::json!("not-a-boc");
        fs::write(&tx_path, tx.to_string()).expect("transaction artifact should be written");
        let artifact = super::SmokeArtifactManifestEntry::new(
            "transaction",
            "transaction.json",
        ...
        assert!(
            gate_failures.iter().any(|failure| {
                failure.contains("transaction artifact transaction.json post shardAccountBoc64")
                    && failure.contains("for tx-a is not a decodable shard account")
            }),
    ...
    #[test]
    fn state_flow_tx_validation_rejects_malformed_rand_seed() {
        let temp_dir = tempfile::tempdir().expect("temp dir should be created");
        let tx_path = temp_dir.path().join("transaction.json");
        let mut tx = sample_state_flow_json("tx-a");
        tx["replay"]["randSeedHex"] = serde_json::json!("ff");
        ...
        assert!(
            gate_failures.iter().any(|failure| {
                failure.contains("transaction artifact transaction.json replay randSeedHex")
                    && failure.contains("for tx-a is not a 32-byte random seed")
            }),
            "expected malformed random seed failure, got {:?}",
            gate_failures
        );
    }
    ...
    #[test]
    fn state_flow_replay_validation_rejects_malformed_shard_account_boc() {
        let temp_dir = tempfile::tempdir().expect("temp dir should be created");
        let replay_path = temp_dir.path().join("replay.json");
        let mut baseline_observation = sample_replay_observation_json(true);
        baseline_observation["state"]["shardAccountBoc64"] = serde_json::json!("not-a-boc");
        fs::write(
            &replay_path,
            serde_json::json!({
                "schemaVersion": 1,
        ...
        assert!(
            gate_failures.iter().any(|failure| {
                failure.contains("replay artifact replay.json baseline state shardAccountBoc64")
                    && failure.contains("for tx-a is not a decodable shard account")
            }),
            "expected replay malformed shard account failure, got {:?}",
            gate_failures
        );
    }
    ...
    fn sample_state_flow_json(query_hash: &str) -> serde_json::Value {
        serde_json::json!({
            ...
            "replay": {
                "mcSeqno": 7,
                "randSeedHex": sample_rand_seed_hex(),
    ...
    fn sample_snapshot_json(state_id: &str, status: &str) -> serde_json::Value {
        serde_json::json!({
            "shardAccountBoc64": test_empty_shard_account_boc64(state_id),
            "lastTransLt": 0,
            "lastTransHash": "00",
            "accountAddress": null,
            "status": status,
    ...
    fn test_empty_shard_account_boc64(state_id: &str) -> String {
        let last_trans_lt = match state_id {
            "post" => 42,
            _ => 0,
        };
        let shard_account = ShardAccount {
            account: Lazy::new(&tycho_types::models::OptionalAccount(None))
                .expect("empty account should fit in a lazy cell"),
    ...
    fn sample_rand_seed_hex() -> String {
        "00".repeat(32)
    }
}
